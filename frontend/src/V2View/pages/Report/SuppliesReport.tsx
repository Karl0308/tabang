...
import { ItemStockHistoryDTO } from "../../objects/ItemStock";
...
const SuppliesReport = () => {
  ...
  const [data, setData] = useState<ItemStockHistoryDTO[]>([]);
  ...
  const fetchData = () => {
    ...
    setIsLoading(true);
    axiosInstance
      .post(APIURLS.report.getSuppliesHistory(), transformedFilter)
      .then((res) => {
        const result = res.data as ItemStockHistoryDTO[];
        setData(result);
      })
      .catch((error) => {
      })
  ...
  const exportTableAsPDF = () => {
    ...
    const columnWidths = [30, 20, 20, 20, 20, 20, 20, 20, 20]; // Adjust based on the number of columns
    ...
    const total = `Total Lines: ${totalRows}`;
    doc.text(total, 10, 25); // Add header at the top of the first page

    doc.setLineWidth(0.5); // Optional: set the line thickness
    doc.line(10, 30, 200, 30); // Draw a horizontal line from x=10 to x=200 at y=32
    ...
    doc.line(
      xPositions[0],
      y,
      xPositions[xPositions.length - 1] + columnWidths[columnWidths.length - 1],
      y
    ); // Draw border at the bottom of the last row
    ...
    const filename = `tabang-supplies-pdf-reports-${formattedDate}.pdf`;
        ...
        <table id="table-to-export" className="w-full border-collapse">
          <thead>
            <tr>
              ...
              >
                Date {orderBy === "date" && (orderDesc ? "↓" : "↑")}
              </th>
              <th
                className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer dark:bg-gray-800 dark:text-white dark:border-gray-700"
                onClick={() => handleOrder("refNo")}
              >
                Ref No {orderBy === "refNo" && (orderDesc ? "↓" : "↑")}
              </th>
              <th
                className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer dark:bg-gray-800 dark:text-white dark:border-gray-700"
                onClick={() => handleOrder("itemName")}
              >
                Item {orderBy === "itemName" && (orderDesc ? "↓" : "↑")}
              </th>
              <th
                className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer dark:bg-gray-800 dark:text-white dark:border-gray-700"
                onClick={() => handleOrder("categoryName")}
              >
                Category {orderBy === "categoryName" && (orderDesc ? "↓" : "↑")}
              </th>
              <th
                className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer dark:bg-gray-800 dark:text-white dark:border-gray-700"
                onClick={() => handleOrder("stockType")}
              >
                Type {orderBy === "stockType" && (orderDesc ? "↓" : "↑")}
              </th>
              <th
                className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer dark:bg-gray-800 dark:text-white dark:border-gray-700"
                onClick={() => handleOrder("quantity")}
              >
                Quantity {orderBy === "quantity" && (orderDesc ? "↓" : "↑")}
              </th>
              <th
                className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer dark:bg-gray-800 dark:text-white dark:border-gray-700"
                onClick={() => handleOrder("previousQuantity")}
              >
                Previous{" "}
                {orderBy === "previousQuantity" && (orderDesc ? "↓" : "↑")}
              </th>
              <th
                className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer dark:bg-gray-800 dark:text-white dark:border-gray-700"
                onClick={() => handleOrder("currentQuantity")}
              >
                Adjusted{" "}
                {orderBy === "currentQuantity" && (orderDesc ? "↓" : "↑")}
              </th>
              <th
                className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer dark:bg-gray-800 dark:text-white dark:border-gray-700"
                onClick={() => handleOrder("userName")}
              >
                Adjust By {orderBy === "userName" && (orderDesc ? "↓" : "↑")}
              </th>
            </tr>
          </thead>
          <tbody>
            {data?.map((item, index) => (
              ...
              >
                ...
                <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 text-left ">
                  {item.stockType === 1 ? "STOCK IN" : "STOCK OUT"}
                </td>
                <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 text-left ">
                  {item.quantity}
                </td>
