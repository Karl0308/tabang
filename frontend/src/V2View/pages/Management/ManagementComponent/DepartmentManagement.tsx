...
export default function DepartmentManagement() {
  ...
  const fetchDepartments = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.post<DepartmentApiResponse>(
        APIURLS.departments.getDepartments(), // replace with your endpoint
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSaving(true);
      await axiosInstance.post(APIURLS.departments.saveDepartment(), formData);
      setShowModal(false);
      fetchDepartments();
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/api/departments/${deleteDepartment.id}`);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border-2 border-gray-200 dark:border-gray-700">
      ...
      <div className="p-6 border-b-2 border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div className="flex items-center gap-3">
          ...
          <p className="text-xl font-bold text-gray-900 dark:text-white">Department Management</p>
        </div>
              ...
              <tr>
                <td colSpan={2} className="text-center py-12">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-gray-500 dark:text-gray-400 font-medium">No departments found</span>
                  </div>
                </td>
              </tr>
            )}
            {departments.map((dept) => (
              <tr key={dept.id} className="group hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 dark:hover:from-gray-700 dark:hover:to-gray-600 transition-all duration-200 hover:shadow-md hover:scale-[1.01] active:scale-[0.99] border-l-4 border-transparent hover:border-blue-500">
                <td className="px-6 py-4 text-gray-900 dark:text-gray-100 font-medium group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors">{dept.name}</td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full transform transition-all duration-300 animate-fadeIn">
            ...
            <div className="p-6 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Delete Department?</h3>
              <p className="text-gray-600 dark:text-gray-400">
                Are you sure you want to delete <span className="font-semibold text-red-600 dark:text-red-400">{deleteDepartment.name}</span>? This action cannot be undone.
              </p>
            </div>
