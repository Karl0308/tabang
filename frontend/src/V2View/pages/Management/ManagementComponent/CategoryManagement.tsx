...
export default function CategoryManagement() {
  ...
  const [formData, setFormData] = useState<Category>({ id: 0, name: "" });
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.post<CategoryApiResponse>(
        APIURLS.categories.getCategories(), // your backend endpoint
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSaving(true);
      await axiosInstance.post(APIURLS.categories.saveCategory(), formData);
      setShowModal(false);
      fetchCategories();
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axiosInstance.delete(APIURLS.categories.deleteCategory() + deleteCategory.id);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border-2 border-gray-200 dark:border-gray-700">
      ...
      <div className="p-6 border-b-2 border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div className="flex items-center gap-3">
          ...
          <p className="text-xl font-bold text-gray-900 dark:text-white">Category Management</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y-2 divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr>
                <td colSpan={2} className="text-center py-12">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-gray-500 dark:text-gray-400 font-medium">No categories found</span>
                  </div>
                </td>
              </tr>
            )}
            ...
            {categories.map((cat) => (
              <tr key={cat.id} className="group hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 dark:hover:from-gray-700 dark:hover:to-gray-600 transition-all duration-200 hover:shadow-md hover:scale-[1.01] active:scale-[0.99] border-l-4 border-transparent hover:border-blue-500">
                <td className="px-6 py-4 text-gray-900 dark:text-gray-100 font-medium group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors">{cat.name}</td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full transform transition-all duration-300 animate-fadeIn">
            ...
            <div className="p-6 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Are you sure you want to delete <span className="font-semibold text-red-600 dark:text-red-400">{deleteCategory.name}</span>? This action cannot be undone.
              </p>
            </div>
