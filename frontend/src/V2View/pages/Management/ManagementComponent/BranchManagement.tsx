...
export default function BranchManagement() {
  ...
  const fetchBranches = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.post<BranchApiResponse>(
        APIURLS.branch.getBranches(), // replace with your backend endpoint
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSaving(true);
      await axiosInstance.post(APIURLS.branch.saveBranch(), formData);
      setShowModal(false);
      fetchBranches();
      showToast(isUpdate ? "Successfully updated!" : "Successfully added!", "success");
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/api/branches/${deleteBranch.id}`);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border-2 border-gray-200 dark:border-gray-700">
      ...
      <div className="p-6 border-b-2 border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div className="flex items-center gap-3">
          ...
          <p className="text-xl font-bold text-gray-900 dark:text-white">Branch Management</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y-2 divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
            <tr>
              {["Name", "Department", "Actions"].map((col) => (
                <th key={col} className="px-6 py-4 text-xs font-bold text-gray-700 dark:text-gray-300 text-left uppercase tracking-wider">{col}</th>
              ))}
            </tr>
          </thead>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr>
                <td colSpan={3} className="text-center py-12">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-gray-500 dark:text-gray-400 font-medium">No branches found</span>
                  </div>
                </td>
              </tr>
            )}
            ...
            {branches.map((branch) => (
              <tr key={branch.id} className="group hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 dark:hover:from-gray-700 dark:hover:to-gray-600 transition-all duration-200 hover:shadow-md hover:scale-[1.01] active:scale-[0.99] border-l-4 border-transparent hover:border-blue-500">
                <td className="px-6 py-4 text-gray-900 dark:text-gray-100 font-medium group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors">{branch.name}</td>
                <td className="px-6 py-4">
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-blue-400 to-indigo-500 text-white shadow-sm">
                    {DepartmentBase[branch.departmentBase]}
                  </span>
                </td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 animate-fadeIn">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  value={formData.departmentBase}
                  onChange={(e) => setFormData({ ...formData, departmentBase: Number(e.target.value) })}
                ...
                >
                  {Object.entries(DepartmentBase)
                    .filter(([key, val]) => typeof val === "number")
                    .map(([key, val]) => (
                      <option key={val as number} value={val as number}>{key}</option>
                    ))}
                </select>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full transform transition-all duration-300 animate-fadeIn">
            ...
            <div className="p-6 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Delete Branch?</h3>
              <p className="text-gray-600 dark:text-gray-400">
                Are you sure you want to delete <span className="font-semibold text-red-600 dark:text-red-400">{deleteBranch.name}</span>? This action cannot be undone.
              </p>
            </div>
