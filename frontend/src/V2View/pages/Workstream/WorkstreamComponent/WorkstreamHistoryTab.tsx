...
import { WorkstreamHistory } from "../../../objects/Workstream";
import userLogo from "../../../../img/user.png";

interface HistoryTabProps {
  workstreamId: string | number;
}

const WorkstreamHistoryTab: React.FC<HistoryTabProps> = ({ workstreamId }) => {
  const [historyList, setHistoryList] = useState<WorkstreamHistory[]>();
  ...
  const FetchHistory = () => {
    setIsLoading(true);
    axiosInstance
      .get(APIURLS.workstream.getWorkstreamHistoriesById() + workstreamId)
      .then((res) => {
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg shadow-md">
      {/* History List */}
      <div className="bg-white dark:bg-gray-700 p-4 rounded-md flex flex-col gap-3">
        {isLoading ? (
                  ...
                  <div className="flex-1 flex flex-col items-start">
                    ...
                    {idx === 0 ? (
                      <p className="text-gray-700 dark:text-gray-200 text-left text-sm sm:text-base break-words whitespace-pre-wrap">
                        <b>{history.userFullName}</b> created this Workstream.
                      </p>
                    ) : (
...
export default WorkstreamHistoryTab;
